...
public class AudioSourceOutputDevicePrefabDemo : MonoBehaviour
{
    ...
    void OnGUI()
    {
        ...
        AudioStreamDemoSupport.GUIHeader(this.fmodVersion);

        GUILayout.Label("This scene demonstrates advanced usage of instantiating a prefab with AudioSourceOutputDevice component attached\r\n- prefab has by default its output device set to 0, the output is changed at runtime based on selection", AudioStreamDemoSupport.guiStyleLabelNormal);
        GUILayout.Label("Select output device and press Instantiate button to create new prefab playing its AudioClip on selected output", AudioStreamDemoSupport.guiStyleLabelNormal);
        ...
        // GUILayout.Label(":", AudioStreamDemoSupport.guiStyleLabelNormal);
        if (GUILayout.Button("Instantiate output device prefab and play its AudioClip on selected output", AudioStreamDemoSupport.guiStyleButtonNormal))
        {
        ...
        if (this.instantiatedOutputs.Count > 0)
            GUILayout.Label("Running instances (playback of each one can be stopped/started individually):", AudioStreamDemoSupport.guiStyleLabelNormal);
