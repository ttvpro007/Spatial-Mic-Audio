...
public class OutputDeviceUnityMixerHotpluggingDemo : MonoBehaviour
{
    ...
    void OnGUI()
    {
        ...
        GUILayout.Label("This scene will play AudioSource in scene via Unity AudioMixer with AudioStreamOutputDevice effect which OutputID is set based on user choice from all displayed devices currently available.", AudioStreamDemoSupport.guiStyleLabelNormal);
        GUILayout.Label("You can un/plug / change default device/s at runtime, the list will update, and internally the mixer effect will be in sync with new device list.", AudioStreamDemoSupport.guiStyleLabelNormal);
        GUILayout.Label("The mixer effect will always play on last selected device ID (even in new list). - you can customize this behaviour in OnDevicesChanged event of AudioStreamDevicesChangedNotify component in the scene.", AudioStreamDemoSupport.guiStyleLabelNormal);
