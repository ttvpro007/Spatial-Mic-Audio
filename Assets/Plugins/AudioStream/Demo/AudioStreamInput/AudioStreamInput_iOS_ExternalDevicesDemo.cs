...
public class AudioStreamInput_iOS_ExternalDevicesDemo : MonoBehaviour
{
    ...
    void OnGUI()
    {
        ...
        GUILayout.Label("This is iOS specific scene which displays and records from all connected devices including external/Bluetooth inputs.\nSee 'iOS recording notes' in Documentation for details.", AudioStreamDemoSupport.guiStyleLabelNormal);
        GUILayout.Label("!! NOTE: Use normal 'AudioStreamInput'/'AudioStreamInput2D' components for devices builtin microphones", AudioStreamDemoSupport.guiStyleLabelNormal);
        GUILayout.Label("The scene only properly works when run on an actual iOS device.", AudioStreamDemoSupport.guiStyleLabelNormal);

        if (Application.platform != RuntimePlatform.IPhonePlayer)
            GUILayout.Label("On all other platforms only testing sine signal is played via a dummy device.", AudioStreamDemoSupport.guiStyleLabelNormal);
        else
            GUILayout.Label("You should be able to dis/connect e.g. Bluetooth headset/ AirPods and select it for recording.", AudioStreamDemoSupport.guiStyleLabelNormal);

        GUILayout.Label("Press 'Update audio session' to update iOS audio session if device is dis/connected and notification is not properly handled (i.e. it doesn't disappear/show up).\r\nThis setup is called automatically on scene load.", AudioStreamDemoSupport.guiStyleLabelNormal);

        if (GUILayout.Button("Update audio session", AudioStreamDemoSupport.guiStyleButtonNormal))
        ...
        if (this.availableInputs.Count > 0)
        {
            ...
            GUILayout.Label(this.audioStreamInput_IOS_ExternalDevices.GetType() + "   ========================================", AudioStreamDemoSupport.guiStyleLabelSmall);
            ...
            GUILayout.Label(Mathf.Round(this.audioStreamInput_IOS_ExternalDevices.gain * 100f) + " %", AudioStreamDemoSupport.guiStyleLabelNormal);
            ...
            if (GUILayout.Button(this.audioStreamInput_IOS_ExternalDevices.isRecording ? "Stop" : "Record", AudioStreamDemoSupport.guiStyleButtonNormal))
                if (this.audioStreamInput_IOS_ExternalDevices.isRecording)
                    this.audioStreamInput_IOS_ExternalDevices.Stop();
                else
