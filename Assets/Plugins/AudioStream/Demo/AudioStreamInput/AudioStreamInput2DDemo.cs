...
public class AudioStreamInput2DDemo : MonoBehaviour
{
    ...
    void OnGUI()
    {
        AudioStreamDemoSupport.GUIHeader(this.audioStreamInput ? " " + this.audioStreamInput.fmodVersion : "");
        AudioStreamDemoSupport.GUIAudioTextures(this.audioTexture_OutputData, this.audioTexture_SpectrumData);

        GUILayout.Label("Choose from available recording devices and press Record.\r\nThe cube will react to sound and signal energy is computed from AudioSource's GetOutputData for each output channel separately.", AudioStreamDemoSupport.guiStyleLabelNormal);
        GUILayout.Label("NOTE: you can un/plug device/s from your system during runtime - the device list will update accordingly", AudioStreamDemoSupport.guiStyleLabelNormal);
        GUILayout.Label("Available recording devices ((*) indicates system default recording input):", AudioStreamDemoSupport.guiStyleLabelNormal);
...

        {
            ...
            GUILayout.Label(this.audioStreamInput.GetType() + "   ========================================", AudioStreamDemoSupport.guiStyleLabelSmall);
            ...
            GUILayout.Label("Signal energy per output channel from GetOutputData: ", AudioStreamDemoSupport.guiStyleLabelNormal);
            ...
            GUILayout.Label(string.Format("Input mixer latency average: {0} ms", this.audioStreamInput.latencyAverage), AudioStreamDemoSupport.guiStyleLabelNormal);
            ...
            if (!this.audioStreamInput.useUnityToResampleAndMapChannels)
            {
                ...
                GUILayout.Label("Current mix matrix (rows - output speakers, columns - input channels):", AudioStreamDemoSupport.guiStyleLabelMiddle);
                ...
                if (mm != null && mm.Rank == 2)
                {
                    for (var row = 0; row < mm.GetLength(0); ++row)
                    {
                        ...
                        GUILayout.Label(srow, AudioStreamDemoSupport.guiStyleLabelSmall);
                    }
                }
            }
            ...
            GUILayout.Label(Mathf.Round(this.audioStreamInput.gain * 100f) + " %", AudioStreamDemoSupport.guiStyleLabelNormal);
            ...
            if (GUILayout.Button(this.audioStreamInput.isRecording ? "Stop" : "Record", AudioStreamDemoSupport.guiStyleButtonNormal))
                if (this.audioStreamInput.isRecording)
                    this.audioStreamInput.Stop();
                else
                    this.audioStreamInput.Record();
            ...
            if (this.audioStreamInput.isRecording)
            {
                if (GUILayout.Button(this.audioStreamInput.isPaused ? "Resume" : "Pause", AudioStreamDemoSupport.guiStyleButtonNormal))
