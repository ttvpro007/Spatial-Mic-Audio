...
public class AudioStreamInputDemo : MonoBehaviour
{
    ...
    void OnGUI()
    {
        ...
        GUILayout.Label("Choose from available recording devices and press Record.\r\nThe cubes will react to sound - signal energy is computed from AudioSource's GetOutputData for each channel separately.\r\nNotice the latency is much higher than in AudioStreamInput2DDemo scene, but the sound is spatialized by Unity spatializer; the Rolloff is set to Linear.", AudioStreamDemoSupport.guiStyleLabelNormal);
        GUILayout.Label("You can move the ball (AudioSource) using [W/A/S/D], reset its position with [R].", AudioStreamDemoSupport.guiStyleLabelNormal);
        GUILayout.Label("For full 3D spatialization see also Resonance components in 'Spatialization' section of the main screen.", AudioStreamDemoSupport.guiStyleLabelNormal);
