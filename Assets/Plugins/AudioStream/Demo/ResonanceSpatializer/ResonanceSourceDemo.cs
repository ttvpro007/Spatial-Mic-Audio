...
public class ResonanceSourceDemo : MonoBehaviour
{
    ...
    void OnGUI()
    {
        AudioStreamDemoSupport.GUIHeader(this.resonanceSource ? " " + this.resonanceSource.fmodVersion : "");
        ...
        GUILayout.Label(this.resonanceSource.GetType() + "   ========================================", AudioStreamDemoSupport.guiStyleLabelSmall);
        ...
        using (new GUILayout.HorizontalScope())
        {
            GUILayout.Label(string.Format("Volume: {0} dB", Mathf.RoundToInt(this.resonanceSource.gain)), AudioStreamDemoSupport.guiStyleLabelNormal, GUILayout.MaxWidth(Screen.width / 2));
        ...
        using (new GUILayout.HorizontalScope())
        {
            ...
            this.resonanceSource.streamType = (AudioStreamBase.StreamAudioType)ComboBoxLayout.BeginLayout(0, System.Enum.GetNames(typeof(AudioStreamBase.StreamAudioType)), (int)this.resonanceSource.streamType, 10, AudioStreamDemoSupport.guiStyleButtonNormal, GUILayout.MaxWidth(Screen.width / 2));
        }

        GUILayout.BeginHorizontal();

        if (GUILayout.Button(this.resonanceSource.isPlaying ? "Stop" : "Play", AudioStreamDemoSupport.guiStyleButtonNormal))
        ...
        if (this.resonanceSource.isPlaying)
        {
            if (GUILayout.Button(this.resonanceSource.isPaused ? "Resume" : "Pause", AudioStreamDemoSupport.guiStyleButtonNormal))
