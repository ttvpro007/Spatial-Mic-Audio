...
public class AudioStreamMinimalStressTest : MonoBehaviour
{
    ...
    void OnGUI()
    {
        AudioStreamDemoSupport.GUIHeader(this.audioStreams != null && this.audioStreams.Count > 0 ? " " + this.audioStreams[0].fmodVersion : "");

        GUILayout.Label("Stress testing scene for the AudioStreamMinimal component", AudioStreamDemoSupport.guiStyleLabelNormal);
        GUILayout.Label(string.Format("{0} AudioStreamMinimal game objects are created at Start and are set to start streaming simultaneously the same url", this.audioStreams.Count), AudioStreamDemoSupport.guiStyleLabelNormal);
        GUILayout.Label("[Depending on the network connection several might not be able to actually connect]", AudioStreamDemoSupport.guiStyleLabelNormal);
        ...
        if (this.allReady)
        {
            ...
            if (GUILayout.Button(atLeastOnePlaying ? "Stop all" : "Start all", AudioStreamDemoSupport.guiStyleButtonNormal))
            {
                foreach (var @as in this.audioStreams)
        ...
        GUILayout.Label(sb.ToString(), AudioStreamDemoSupport.guiStyleLabelNormal);
        ...
        foreach (var audioStream in this.audioStreams)
        {
            ...
            GUILayout.Label(audioStream.gameObject.name, AudioStreamDemoSupport.guiStyleLabelNormal, GUILayout.MaxWidth(Screen.width / 2));
            // GUILayout.Label(audioStream.url, AudioStreamDemoSupport.guiStyleLabelNormal);
            ...
            if (GUILayout.Button(audioStream.isPlaying ? "Stop" : "Play", AudioStreamDemoSupport.guiStyleButtonNormal, GUILayout.MaxWidth(Screen.width / 2)))
            ...
            if (audioStream.isPlaying)
            {
                if (GUILayout.Button(audioStream.isPaused ? "Resume" : "Pause", AudioStreamDemoSupport.guiStyleButtonNormal, GUILayout.MaxWidth(Screen.width / 2)))
