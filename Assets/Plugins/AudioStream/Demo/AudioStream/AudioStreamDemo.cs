...
public class AudioStreamDemo : MonoBehaviour
{
    ...
    {
        this.streamsStatesFromEvents[goName] = "stopped" + (!string.IsNullOrEmpty(downloadedFilepath) ? ", file saved to " + downloadedFilepath : null);
    ...
    void OnGUI()
    {
        AudioStreamDemoSupport.GUIHeader(this.audioStreams != null && this.audioStreams.Length > 0 ? " " + this.audioStreams[0].fmodVersion : "");
        AudioStreamDemoSupport.GUIAudioTextures(this.audioTexture_OutputData, this.audioTexture_SpectrumData);

        GUILayout.Label("Press Play to play entered stream.\r\nURL can be pls/m3u/8 playlist, file URL, or local filesystem path (with or without the 'file://' prefix)", AudioStreamDemoSupport.guiStyleLabelNormal);
        GUILayout.Label("-- AudioStream component plays audio via Unity's AudioSource/AudioClip\r\n-- AudioStreamMinimal bypasses Unity audio playing directly via FMOD", AudioStreamDemoSupport.guiStyleLabelNormal);
        ...
        for (var aidx = 0; aidx < this.audioStreams.Length; ++aidx)
        {
            ...
            GUILayout.Label(audioStream.GetType() + "   ========================================", AudioStreamDemoSupport.guiStyleLabelSmall);
            ...
            {
                GUILayout.Label("Stream url: ", AudioStreamDemoSupport.guiStyleLabelNormal, GUILayout.MaxWidth(Screen.width / 2));
            ...
            using (new GUILayout.HorizontalScope())
            {
                GUILayout.Label("Decoder 'blockalign' size (bytes): ", AudioStreamDemoSupport.guiStyleLabelNormal, GUILayout.MaxWidth(Screen.width / 2));
            ...
            using (new GUILayout.HorizontalScope())
            {
                GUILayout.Label(string.Format("'blockalign multiplier for download' (times): {0}", audioStream.blockalignDownloadMultiplier), AudioStreamDemoSupport.guiStyleLabelNormal, GUILayout.MaxWidth(Screen.width / 2));
                audioStream.blockalignDownloadMultiplier = (uint)GUILayout.HorizontalSlider(audioStream.blockalignDownloadMultiplier, 1, 16);
            }

            GUILayout.Label(string.Format("Initial download until playback attempt is started: {0} b", audioStream.blockalign * audioStream.blockalignDownloadMultiplier), AudioStreamDemoSupport.guiStyleLabelNormal);

            using (new GUILayout.HorizontalScope())
            {
                GUILayout.Label(string.Format("Stream format{0}: ", audioStream.streamType == AudioStreamBase.StreamAudioType.AUTODETECT ? " (autodetection fails on iOS often - please select stream type explicitely on iOS)" : ""), AudioStreamDemoSupport.guiStyleLabelNormal, GUILayout.MaxWidth(Screen.width / 2));
                audioStream.streamType = (AudioStreamBase.StreamAudioType)ComboBoxLayout.BeginLayout(aidx, System.Enum.GetNames(typeof(AudioStreamBase.StreamAudioType)), (int)audioStream.streamType, 10, AudioStreamDemoSupport.guiStyleButtonNormal, GUILayout.MaxWidth(Screen.width / 2));
            }

            // dont have style for toggle - leave always on in demo
            //GUILayout.Space(10);
            //audioStream.continuosStreaming = GUILayout.Toggle(audioStream.continuosStreaming, "Auto reconnect/play again when the connection is dropped/finished.", AudioStreamDemoSupport.guiStyleLabelNormal);
            ...
            if (audioStream.streamType == AudioStreamBase.StreamAudioType.RAW)
            {
                ...
                using (new GUILayout.VerticalScope())
                {
                    using (new GUILayout.HorizontalScope())
                    {
                        GUILayout.Label("RAW Format: ", AudioStreamDemoSupport.guiStyleLabelNormal, GUILayout.MaxWidth(Screen.width / 2));
                        audioStream.RAWSoundFormat = (FMOD.SOUND_FORMAT)GUILayout.SelectionGrid((int)audioStream.RAWSoundFormat, System.Enum.GetNames(typeof(FMOD.SOUND_FORMAT)), 5, AudioStreamDemoSupport.guiStyleButtonNormal);
                    }

                    using (new GUILayout.HorizontalScope())
                    {
                        GUILayout.Label("RAW Samplerate: ", AudioStreamDemoSupport.guiStyleLabelNormal, GUILayout.MaxWidth(Screen.width / 2));
                    ...
                    using (new GUILayout.HorizontalScope())
                    {
                        GUILayout.Label("RAW Channels: ", AudioStreamDemoSupport.guiStyleLabelNormal, GUILayout.MaxWidth(Screen.width / 2));
            ...
            if (audioStream.url.ToLowerInvariant().StartsWith("http"))
            {
                using (new GUILayout.HorizontalScope())
                {
                    GUILayout.Label(string.Format("Play cached/downloaded media for entered Url from local storage if it exists instead of live audio")
                        , AudioStreamDemoSupport.guiStyleLabelNormal, GUILayout.MaxWidth(Screen.width / 2));
                    ...
                    using (new GUILayout.VerticalScope(GUILayout.MaxWidth(Screen.width / 2)))
                    {
                        ...
                        if (audioStream.playFromCache)
                        {
                            ...
                            GUILayout.Label(string.Format("[The file will be played from '{0}' if it exists]", fname), AudioStreamDemoSupport.guiStyleLabelNormal);
                        }
                    }
                }
                ...
                using (new GUILayout.HorizontalScope())
                {
                    ...
                    using (new GUILayout.VerticalScope(GUILayout.MaxWidth(Screen.width / 2)))
                    {
                        ...
                        if (audioStream.downloadToCache)
                            GUILayout.Label(string.Format("[info] temporary cache at: {0}]", RuntimeSettings.temporaryDirectoryPath), AudioStreamDemoSupport.guiStyleLabelNormal);
                    }
                }
            }
            ...
            using (new GUILayout.HorizontalScope())
            {
                ...
                GUILayout.Label(string.Format("[Allocated / Available for playback (bytes): {0} / {1}]", audioStream.mediaCapacity, audioStream.mediaAvailable), AudioStreamDemoSupport.guiStyleLabelNormal, GUILayout.MaxWidth(Screen.width / 2));
            ...
            using (new GUILayout.HorizontalScope())
            {
                if (audioStream is AudioStream.AudioStream)
                {
                    ...
                    GUILayout.Label(string.Format("Volume: {0} %", Mathf.Round(asource.volume * 100f)), AudioStreamDemoSupport.guiStyleLabelNormal, GUILayout.MaxWidth(Screen.width / 2));
                ...
                else
                {
                    ...
                    GUILayout.Label(string.Format("Volume: {0} %", Mathf.Round(@as.volume * 100f)), AudioStreamDemoSupport.guiStyleLabelNormal, GUILayout.MaxWidth(Screen.width / 2));
            ...
            using (new GUILayout.HorizontalScope())
            {
                if (GUILayout.Button(audioStream.isPlaying ? "Stop" : "Play", AudioStreamDemoSupport.guiStyleButtonNormal))
                {
                ...
                if (audioStream.isPlaying)
                {
                    if (GUILayout.Button(audioStream.isPaused ? "Resume" : "Pause", AudioStreamDemoSupport.guiStyleButtonNormal))
                    ...
                    {
                        if (GUILayout.Button("<< 30 sec", AudioStreamDemoSupport.guiStyleButtonNormal))
            ...
            if (this.tags.TryGetValue(audioStream.gameObject.name, out _tags))
            {
                ...
                foreach (var d in _tags)
                    if (d.Value is string)
                        GUILayout.Label(d.Key + ": " + d.Value, AudioStreamDemoSupport.guiStyleLabelNormal, GUILayout.MaxWidth(Screen.width / 2));
                GUILayout.EndVertical();

                foreach (var d in _tags)
                    if (d.Value is Texture2D)
                        GUILayout.Label(d.Value as Texture2D, AudioStreamDemoSupport.guiStyleLabelNormal, GUILayout.Width(Screen.width / 10), GUILayout.Height(Screen.height / 10));
