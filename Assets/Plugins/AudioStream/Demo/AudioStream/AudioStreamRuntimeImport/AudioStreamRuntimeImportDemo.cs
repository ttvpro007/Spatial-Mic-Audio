...
public class AudioStreamRuntimeImportDemo : MonoBehaviour
{
    ...
    void OnGUI()
    {
        AudioStreamDemoSupport.GUIHeader(this.asImport != null ? " " + this.asImport.fmodVersion : "");
        ...
        GUILayout.Label("This component is primarily aimed at downloading files via *non realtime* speeds which are (much) higher than real time playback - this means that real time streams such as net radio streams most likely won't work", AudioStreamDemoSupport.guiStyleLabelNormal);
        GUILayout.Label("^ (for normal downloading see AudioStreamDemo scene which decodes (only) in realtime thus slower than this and allows audio to be played while it's downloading", AudioStreamDemoSupport.guiStyleLabelNormal);
        GUILayout.Label("NOTE: URL of a file placed in application's StreamingAssets is used in this demo (which kind of defeats the purpose since local file is read directly normally) - enter URL of a file on LAN or on a quick enough network to test properly", AudioStreamDemoSupport.guiStyleLabelNormal);
        ...
        GUILayout.Label("AudioStreamRuntimeImport will download/stream, decode audio and save it into cache associated w/ url instead of playing it; once done/stopped it will then construct an AudioClip and pass it to user AudioSource for playback", AudioStreamDemoSupport.guiStyleLabelNormal);
        GUILayout.Label("The downloaded AudioClip data is temporary, but is not deleted after the AudioClip creation (note these files can get quite large); while they're in temporary cache they can be played later offline if/when needed", AudioStreamDemoSupport.guiStyleLabelNormal);
        GUILayout.Label("Temporary PCM data used for AudioClip stored as .RAW file in " + RuntimeSettings.temporaryDirectoryPath, AudioStreamDemoSupport.guiStyleLabelNormal);
        ...
        GUILayout.Label(this.asImport.GetType() + "   ========================================", AudioStreamDemoSupport.guiStyleLabelSmall);

        using (new GUILayout.HorizontalScope())
        {
            GUILayout.Label("Stream: ", AudioStreamDemoSupport.guiStyleLabelNormal, GUILayout.MaxWidth(Screen.width / 2));
        ...
        GUILayout.Label("Volume: ", AudioStreamDemoSupport.guiStyleLabelNormal);

        this.userAudioSource.volume = GUILayout.HorizontalSlider(this.userAudioSource.volume, 0f, 1f);
        GUILayout.Label(Mathf.Round(this.userAudioSource.volume * 100f) + " %", AudioStreamDemoSupport.guiStyleLabelNormal);
        ...
        using (new GUILayout.HorizontalScope())
        {
            GUILayout.Label("Stream format: ", AudioStreamDemoSupport.guiStyleLabelNormal, GUILayout.MaxWidth(Screen.width / 2));
            this.asImport.streamType = (AudioStreamBase.StreamAudioType)ComboBoxLayout.BeginLayout(0, System.Enum.GetNames(typeof(AudioStreamBase.StreamAudioType)), (int)this.asImport.streamType, 10, AudioStreamDemoSupport.guiStyleButtonNormal, GUILayout.MaxWidth(Screen.width / 2));
        }

        GUILayout.BeginHorizontal();

        var dl = this.asImport.overwriteCachedDownload ? "Download" : "Retrieve from cache";
        if (GUILayout.Button(this.asImport.isPlaying ? "Stop Download" : dl + (this.playClipAfterDownload ? " and Play" : ""), AudioStreamDemoSupport.guiStyleButtonNormal))
        ...
        if (this.asImport.isPlaying)
        {
            if (GUILayout.Button(this.asImport.isPaused ? "Resume Download" : "Pause Download", AudioStreamDemoSupport.guiStyleButtonNormal))
        ...
        GUILayout.Label(string.Format("Cached file samplerate: {0}", this.asImport.streamSampleRate), AudioStreamDemoSupport.guiStyleLabelNormal);
        GUILayout.Label(string.Format("Cached file channels  : {0}", this.asImport.streamChannels), AudioStreamDemoSupport.guiStyleLabelNormal);
        ...
        if (this.userAudioSource.clip != null)
        {
            ...
            GUILayout.Label(string.Format("Downloaded AudioClip channels: {0}, length: {1} s, playback position: {2:F2} s", this.userAudioSource.clip.channels, this.userAudioSource.clip.length, this.userAudioSource.time), AudioStreamDemoSupport.guiStyleLabelNormal);

            GUILayout.BeginHorizontal();

            if (GUILayout.Button(this.userAudioSource.isPlaying || this.@as_isPaused ? "Stop Playback" : "Play", AudioStreamDemoSupport.guiStyleButtonNormal))
            {
            ...
            if (this.userAudioSource.isPlaying || this.@as_isPaused)
            {
                if (GUILayout.Button(this.@as_isPaused ? "Resume Playback" : "Pause Playback", AudioStreamDemoSupport.guiStyleButtonNormal))
                    if (this.@as_isPaused)
                    {
