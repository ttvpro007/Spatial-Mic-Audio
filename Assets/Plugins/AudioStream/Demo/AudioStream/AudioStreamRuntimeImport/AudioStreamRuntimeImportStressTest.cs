...
public class AudioStreamRuntimeImportStressTest : MonoBehaviour
{
    ...
    void OnGUI()
    {
        AudioStreamDemoSupport.GUIHeader(this.audioStreamImports.Count < 1 ? "" : this.audioStreamImports[0].fmodVersion);

        // AudioStreamImportStressTestDemo

        GUILayout.Label("Stress testing scene for the AudioStreamRuntimeImport component", AudioStreamDemoSupport.guiStyleLabelNormal);
        ...
        if (this.allReady)
        {
            ...
            if (GUILayout.Button(atLeastOnePlaying ? "Stop all downloads" : "Start all downloads", AudioStreamDemoSupport.guiStyleButtonNormal))
            {
                foreach (var asd in this.audioStreamImports)
        ...
        for (var i = 0; i < this.audioStreamImports.Count; ++i)
        {
            ...
            using (new GUILayout.HorizontalScope())
            {
                if (GUILayout.Button(asd.isPlaying ? "Stop Download" : "Download" + (this.playClipAfterDecoding ? " and Play" : ""), AudioStreamDemoSupport.guiStyleButtonNormal))
                    if (asd.isPlaying)
                        asd.Stop();
                    else
                    {
