...
public class AudioStreamMemoryDemo : MonoBehaviour
{
    ...
    void OnGUI()
    {
        AudioStreamDemoSupport.GUIHeader(this.asMemory != null ? " " + this.asMemory.fmodVersion : "");
        ...
        GUILayout.Label("Press Decode to start decoding demo track from memory\r\nDemo will read the file into a memory buffer, AudioStreamMemory will then read and decode audio from it; once done/stopped it will then construct an AudioClip and pass it to user AudioSource for playback\r\n" +
            "The whole decoding and new clip creation is done in-memory (can optionally use disk cache)", AudioStreamDemoSupport.guiStyleLabelNormal);
        ...
        GUILayout.Label(this.asMemory.GetType() + "   ========================================", AudioStreamDemoSupport.guiStyleLabelSmall);
        ...
        {
            GUILayout.Label("Reader buffer size (bytes): ", AudioStreamDemoSupport.guiStyleLabelNormal);
            if (uint.TryParse(
        ...
        */
        ...
        if (this.userAudioSource)
        {
            using (new GUILayout.HorizontalScope())
            {
                GUILayout.Label(string.Format("Volume: {0} %", Mathf.Round(this.userAudioSource.volume * 100f)), AudioStreamDemoSupport.guiStyleLabelNormal, GUILayout.MaxWidth(Screen.width / 2));
        ...
        GUILayout.Label(string.Format("Decoded bytes: {0} (encoded memory size: {1})", this.asMemory.decoded_bytes, this.asMemory.memoryLength + " b"), AudioStreamDemoSupport.guiStyleLabelNormal);
        GUILayout.Label(string.Format("Last decoding + AudioClip creation took: {0} ms", this.asMemory.decodingToAudioClipTimeInMs), AudioStreamDemoSupport.guiStyleLabelNormal);

        GUILayout.Label("You can enter a unique cache id, subsequent Plays will go straight into cache instead of memory decoding: ", AudioStreamDemoSupport.guiStyleLabelNormal);
        this.asMemory.cacheIdentifier = GUILayout.TextField(this.asMemory.cacheIdentifier);

        if (this.bytes == null)
        {
            GUILayout.Label("Waiting for file to load into memory....", AudioStreamDemoSupport.guiStyleLabelNormal);
        }
        else
        {
            GUILayout.BeginHorizontal();

            if (GUILayout.Button(this.asMemory.isPlaying ? "Stop Decoding" : "Decode" + (this.playClipAfterDecoding ? " and Play" : ""), AudioStreamDemoSupport.guiStyleButtonNormal))
        ...
        if (this.userAudioSource != null && this.userAudioSource.clip != null)
        {
            ...
            GUILayout.Label(string.Format("Decoded AudioClip channels: {0}, length: {1} s, playback position: {2:F2} s", this.userAudioSource.clip.channels, this.userAudioSource.clip.length, this.userAudioSource.time), AudioStreamDemoSupport.guiStyleLabelNormal);
        ...
        GUILayout.Label("Audio track used in this demo:", AudioStreamDemoSupport.guiStyleLabelNormal);
        GUILayout.Label("Shibuya by Electronic Senses | https://soundcloud.com/electronicsenses", AudioStreamDemoSupport.guiStyleLabelNormal);
        GUILayout.Label("Music promoted by https://www.free-stock-music.com", AudioStreamDemoSupport.guiStyleLabelNormal);
        GUILayout.Label("Creative Commons Attribution - ShareAlike 3.0 Unported", AudioStreamDemoSupport.guiStyleLabelNormal);
        GUILayout.Label("https://creativecommons.org/licenses/by-sa/3.0/deed.en_US", AudioStreamDemoSupport.guiStyleLabelNormal);
        GUILayout.Label("audio file is read from StreamingAssets", AudioStreamDemoSupport.guiStyleLabelNormal);
